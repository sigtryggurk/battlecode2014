...
public class Pathing
{
    ...
    public static MapLocation findNewClosestMaxCowGrowth(RobotController rc)
    {
        ...
        if (x != -1 && y != -1)
        {
            ...
            seenCowGrowth.add(VectorTools.locationToInt(loc));
            for(Direction dir:Direction.values()){
                seenCowGrowth.add(VectorTools.locationToInt(loc.add(dir)));
                seenCowGrowth.add(VectorTools.locationToInt(loc.add(dir,2)));
            }
            return loc;
        }
        return null;
